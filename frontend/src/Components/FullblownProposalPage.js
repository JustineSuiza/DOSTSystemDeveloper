...
const FullblownProposalPage = ({ sidebarExpanded }) => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('fullblownProposals');
    if (stored) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('fullblownProposals', JSON.stringify(rows));
  }, [rows]);
  ...
  const exportToExcel = () => {
    ...
    a.href = url;
    a.download = 'FullblownProposals.xlsx';
    a.click();
    URL.revokeObjectURL(url);
  };

  const normalizeImportedFullblownProposalRows = (rows = []) => {
  ...
  const importFromExcel = async (event) => {
    ...
    const processWorkbook = (workbook) => {
      ...
      const importedRows = normalizeImportedFullblownProposalRows(jsonData);
      ...
      alert(`Fullblown proposals imported successfully (${importedRows.length} rows processed).`);
    };
    ...
    try {
      ...
      console.error('Error importing fullblown proposals:', error);
      alert('Error importing fullblown proposals: ' + (error && error.message ? error.message : error));
    } finally {
      event.target.value = '';
    }
  };
  ...
  const handleDelete = async (row) => {
    if (window.confirm(`Delete fullblown proposal "${row.proposalTitle || 'this proposal'}"?`)) {
      try {
        ...
        console.error('Error archiving fullblown proposal:', error);
  ...
  return (
    <article className="pt-5 pb-5 pe-5 ps-4">
      <div className="d-flex justify-content-between align-items-center flex-wrap">
        <label className='h5 fw-semibold pt-2'>Fullblown Proposal</label>
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          <div className='sample me-3' style={{ borderRadius: '50px', padding: '7px 2px 2px 2px' }}>
            <input id="importFullblownProposalFile" type="file" accept=".xlsx,.xls,.csv" onChange={importFromExcel} style={{ display: 'none' }} />
            <button type="button" className="btn border-0" onClick={() => document.getElementById('importFullblownProposalFile').click()}>
          ...
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <form className="modal-content" onSubmit={handleAdd}>
              <div className="modal-header">
                <h5 className="modal-title">Add Fullblown Proposal</h5>
        ...
        <div className="modal fade show" style={{ display: 'block', backgroundColor: 'rgba(0,0,0,0.4)' }}>
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Fullblown Proposal Details</h5>
        ...
        <div className="modal fade show" style={{ display: 'block', backgroundColor: 'rgba(0,0,0,0.4)' }}>
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <form className="modal-content" onSubmit={saveEdit}>
              <div className="modal-header">
                <h5 className="modal-title">Edit Fullblown Proposal</h5>
...
export default FullblownProposalPage;
